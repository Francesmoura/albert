// Deposit.java
// Representa uma transação de depósito do ATM
...
public class Deposit extends Transaction {
    ...
    public void execute() {
        ...
        if (amount != CANCELED) {
            // solicita o envelope de depósito
            ...
            if (envelopeReceived) {
                screen.displayMessageLine("\nYour envelope has been received. " +
                    ...
                    "verify the amount of any enclosed cash and your checks clear.");
                
                // credita na conta para refletir o depósito
                bankDatabase.credit(getAccountNumber(), amount);
            } else { 
                // envelope de depósito não foi recebido
    ...
    // solicita que o usuário insira uma quantia em centavos
    private double promptForDepositAmount() {
        Screen screen = getScreen(); // obtém a referência à tela

        // exibe a solicitação
        screen.displayMessage("\nPlease enter a deposit amount in CENTS (or 0 to cancel): ");
        int input = keypad.getInput(); // recebe a entrada da quantia
